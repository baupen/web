...
class DispatchController extends ApiController
{
    ...
    private function sendMail(Craftsman $craftsman, CurrentIssueState $state, ConstructionSite $constructionSite, EmailServiceInterface $emailService, TranslatorInterface $translator)
    {
        ...
        $email->setActionLink($this->generateUrl('external_share_craftsman', ['identifier' => $craftsman->getEmailIdentifier(), 'writeAuthenticationToken' => $craftsman->getWriteAuthorizationToken()], UrlGeneratorInterface::ABSOLUTE_URL));
        $this->fastSave($email);
